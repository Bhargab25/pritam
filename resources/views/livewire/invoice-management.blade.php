...
<x-mary-card class="mb-6">
    <div class="grid grid-cols-1 md:grid-cols-7 gap-4">
        ...
        <div class="flex items-end">
            <x-mary-checkbox
                label="Show all"
                wire:model.live="showAllFinancialYear" />
        </div>
    </div>
</x-mary-card>
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-12 gap-3 items-end p-4 bg-base-100 rounded-lg border" wire:key="item-{{ $index }}">
                ...
                <div class="col-span-2">
                    ...
                    $stockQty = $product ? $product->stock_quantity : null; // adjust field name
                    @endphp

                    <div class="form-control">
                        <label class="label">
                            <span class="label-text text-xs">Unit / Stock</span>
                        </label>
                        <div class="flex items-center justify-between px-2 py-2 text-sm font-medium">
                            <span>{{ strtoupper($productUnit) }}</span>
                            @if(!is_null($stockQty))
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="space-y-2">
                ...
                <div class="flex justify-between text-sm mt-2">
                    ...
                    <span>+₹{{ number_format($coolieExpense, 2) }}</span>
                </div>
                ...
                <div class="border-t pt-2 mt-2">
                    <div class="flex justify-between text-xl font-bold">
                        ...
                        <span class="text-success">
                            ₹{{ number_format($this->invoiceGrandTotal + $coolieExpense, 2) }}
                        </span>
                    </div>
                </div>
                @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="p-4 bg-base-100 rounded-lg">
                ...
                <div class="space-y-2">
                    ...
                    {{-- Coolie / Delivery --}}
                    @if($viewingInvoice->coolie_expense > 0)
                    <div class="flex justify-between">
                        <span>
                            @if($viewingInvoice->invoice_type === 'cash')
                            Coolie Expense:
                            @else
                            Delivery Charge:
                            @endif
                        </span>
                        <span>₹{{ number_format($viewingInvoice->coolie_expense, 2) }}</span>
                    </div>
                    @endif

                    {{-- Final amount (goods + tax + coolie) --}}
                    <div class="flex justify-between font-bold text-lg border-t pt-2">
                        <span>Total Amount:</span>
                        <span>
                            ₹{{ number_format($viewingInvoice->final_amount ?? $viewingInvoice->total_amount, 2) }}
                        </span>
                    </div>
